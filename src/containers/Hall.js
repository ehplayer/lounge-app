...
class HallContainer extends React.Component {
  ...
  fetchUniv = (currentUnivId, member) => {
    this.props.getUnivTotal(currentUnivId, member, 'hall')
      .catch((err) => {
        console.log(`Error: ${err}`);
        return this.props.setError(err);
      });
    return this.props.getArticleList(currentUnivId, member, 'hall');
  }

  render = () => {
    const { Layout, hall, match, member, status} = this.props;
