...
import {Body, Card, CardItem, Container, Content, Left, List, ListItem, Right, Text, Thumbnail} from 'native-base';
...
class Scheduler extends React.Component {
  ...
  render() {
    const {loading, userList, getMemberListMoreData} = this.props;
    if (loading) return <Loading/>;
    const lastMember = userList[userList.length - 1];
    return (
      <Container>
          <List style={{backgroundColor: '#ffffff'}}>
              <FlatList
                  data={userList}
                  ListHeaderComponent={() => <ListItem style={{height: 50, marginLeft:20, marginRight:0}}><Text style={{width: '100%'}}>원우 목록</Text></ListItem>}
                  ListEmptyComponent={() =>
                  ...
                  renderItem={({item, index}) => {
                      ...
                      return <ListItem avatar style={{height:70, marginLeft:20, marginRight:0,justifyContent:'center', borderBottomWidth:(index === userList.length - 1 ? 0 : 0.3), borderBottomColor:'#cccccc'}}
                  ...
                  onEndReached={() => getMemberListMoreData(lastMember)}
              />
          </List>
